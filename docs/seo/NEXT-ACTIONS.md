...
1. **n8n:** the live workflow is replaced and published; execution 4376 and GitHub run 34394381074 succeeded and produced draft PR #21. Review that draft's sources and device instructions before publication; then inspect the next scheduled execution. See `automation/README.md` for the exact live version and evidence. The active queue now has four focused briefs; the legacy 216-topic backlog is not consumed.
